...
/*   Updated: 2023/06/29 05:34:11 by vchakhno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	ft_f32_print_numerical_part(
	t_any_ostream *any_stream, t_f32 value, t_i8 log
) {
	t_i8	i;
	t_f32	mask;
	t_i8	snapped_log;
	...
	while (i < 4)
	{
		if (!ft_ostream_write_byte(
				any_stream, ft_f32_mod(value / mask, 10) + '0'))
			return (false);
...
bool	ft_f32_oprint(t_any_str_ostream *any_stream, t_f32 value)
{
	t_i8	log;
	...
	log = ft_f32_ilog(value, 10);
	if (!ft_f32_print_numerical_part(any_stream, value, log))
		return (false);
	if (log < 0 || 3 <= log)
	{
		if (!ft_ostream_write(any_stream, "e", 1))
			return (false);
		if (!ft_i8_oprint(any_stream, (log - 2 * (log < 0)) / 3 * 3))
			return (false);
	}
	return (true);
}

bool	ft_f32_oprintln(t_any_str_ostream *any_stream, t_f32 value)
{
	if (!ft_f32_oprint(any_stream, value))
		return (false);
